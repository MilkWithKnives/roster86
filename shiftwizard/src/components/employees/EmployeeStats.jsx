...
export default function EmployeeStats({ employees }) {
  ...
  const totalMaxHours = activeEmployees.reduce((sum, emp) => sum + (emp.max_hours_per_week || 0), 0);
  const avgWage = activeEmployees.length > 0 
    ? activeEmployees.reduce((sum, emp) => sum + (emp.hourly_rate || 0), 0) / activeEmployees.length 
    : 0;
